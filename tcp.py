...
def get_temp_and_humidity(ip='192.168.18.235', port=9999):
    """
    获取温湿度数据
    :param ip: TCP连接的ip地址
    ...
    return (str(round(temp, 2)), str(round(humidity, 2)))


def get_electricity(ip='192.168.18.236', port=23):
    """
    通过TCP连接某个电表，获取电量
    :param ip: TCP连接的ip地址
...
def get_total_electricity():
    ...
    total += get_electricity(port=NHM6300_port)
    total += get_electricity(port=H6000_port)
    total += get_electricity(port=H8000_port)
...
def insert_data(table_name, data):
    ...
    try:
        if table_name == 'temperature':
            cursor.execute("INSERT INTO temperature (temperature, create_time) VALUES (%s, %s)", (data[0], create_time))
